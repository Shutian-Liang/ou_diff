...
class GaussianDiffusion(nn.Module):
    ...
    def ou_noise(self, x_start):
        # x_start : [b, c, t, h, w]
        B, c, t, h, w = x_start.shape
        b = B//4
        device = x_start.device
        gamma = math.exp(-self.theta*self.dt)
        noise = torch.randn(t, c, h, w, device=device)
        # ou process to generate noise sequence
        ou_noise = [noise]
        gamma = math.exp(-self.theta*self.dt)
        #print(f"Gamma: {gamma}, D: {self.D}, Theta: {self.theta}, DT: {self.dt}")  
        for _ in range(b):
            noise = torch.randn(t, c, h, w, device=device)
            for i in range(4):
                noise = gamma * noise + math.sqrt(self.D/self.theta*(1-gamma**2)) * torch.randn_like(noise).to(device)
        ...
        # ou_noise = []
        # for _ in range(b):
        #     noise = torch.randn(c, t, h, w, device=device)
        #     for i in range(4):
        #         noise = gamma * noise + math.sqrt(self.D/self.theta*(1-gamma**2)) * torch.randn_like(noise).to(device)
        #         ou_noise.append(noise)
        # ou_noise = torch.stack(ou_noise) # 4b c t h w
        
        # # [t,b,c,h,w] -> [b,c,t,h,w]
        # ou_noise = ou_noise.permute(1,2,0,3,4)
    ...
    def p_sample(self, x, t, cond = None, cond_scale = 1., clip_denoised = False,
                 sampling_method = 'sequence'):
        ...
        elif sampling_method == 'sequence':
            noise = self.ou_noise(x)
        else:
    ...
    def p_sample_loop(self, shape, cond = None, cond_scale = 1.,
        ...
        if sampling_method == 'independent':
            ...
            img = self.ou_noise(torch.zeros(shape, device=device))
        else:
            raise NotImplementedError
    ...
    def p_losses(self, x_start, t, cond = None, noise = None, **kwargs):
        ...
        x_recon = self.model(x_noisy, t, cond = cond, **kwargs)
